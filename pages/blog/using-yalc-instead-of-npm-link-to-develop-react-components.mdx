...
`MyComponent` now and do a `npm run build` in `my-component`!
...
if it was in there though, adding it via npm link creats a symlink inside of
...
synlink. It'll treat the folder like any other application and look for
...
From there, running `npm start` in `cra-app` will yeild a working component with
...
- `yalc publish` -> Writes to a local npm registry
...
- `yalc link` -> Similar to `npm link` but only links the files that would be in
  the remote npm. Does not persist between `npm install` runs.
...
propegation of updates to where the package has been added or linked. Try
changing the component in `component-lib` while CRA is running and run the
following command and notice the accompying output in the terminal:
